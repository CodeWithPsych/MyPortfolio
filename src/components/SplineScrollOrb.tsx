...
export function SplineOrbProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const place = () => {
      ...
      for (let i = 0; i < waypoints.length - 1; i++) {
        if (
          viewCenter >= waypoints[i].center &&
          viewCenter <= waypoints[i + 1].center
        ) {
          from = waypoints[i];
          to = waypoints[i + 1];
          break;
      ...
      const t =
        span > 0
          ? Math.max(0, Math.min(1, (viewCenter - from.center) / span))
          : 0;
  ...
  return (
    <OrbContext.Provider value={{ registerSlot, unregisterSlot }}>
      ...
      {/* mounted && !isMobile ensures Spline only runs client-side on desktop */}
      {mounted && !isMobile && (
        <div
          ref={orbRef}
          style={{
